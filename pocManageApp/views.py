...
def pluginConfig_insert(request):
    try:
        ...
        if count == 0 and count2 == 0:
            try:
                # 上传文件
                path = os.getcwd() + "/poc/" + plu_dir.name
...
def pluginConfig_update_show(request):
    ...
    # 获取应用类型
    app_list = Plugin_app_db.objects.all()
    return render(request, "plugin_app/plugin_config_update.html",
                  {'plu': plu, 'vul_type': vul_type, 'app_list': app_list})
